/*
==============================================================================
== OntoRelCat : ontorelcat_api_ontorelquery_pub.sql
...
==============================================================================
*/
-- Création du rôle pour accéder au schéma 'ontorelcat_api_ontorelquery'
create role ontorelcat_api_ontorelquery;
comment on role ontorelcat_api_ontorelquery is $$
Groupe des utilisateurs pouvant utiliser l’interface cliente du module "ontorelquery".
Cette API est en lecture seule et sert uniquement à interroger les données.
$$;

-- Création du schéma 'ontorelcat_api_ontorelquery'
create schema if not exists ontorelcat_api_ontorelquery;
comment on schema ontorelcat_api_ontorelquery is $$
Modélisation du catalogue d'OntoRelA - section api query.
Il s'agit de l'API utilisée par OntoRelQuery pour consulter les données d'OntoRel sans les modifier.
$$;

-- Attribution des privilèges
grant usage on schema ontorelcat_api_ontorelquery to ontorelcat_api_ontorelquery;

-- Héritage des privilèges communs (si un rôle public existe)
grant ontorelcat_pub to ontorelcat_api_ontorelquery;

-- Accès lecture seule sur toutes les tables existantes
grant select on all tables in schema ontorelcat_api_ontorelquery to ontorelcat_api_ontorelquery;

-- Privilèges par défaut pour les futures tables
alter default privileges in schema ontorelcat_api_ontorelquery
grant select on tables to ontorelcat_api_ontorelquery;

-- ===========================================================================
-- création des types
-- ===========================================================================

-- Suppression du type 'table_id_result' s'il existe déjà dans le schéma 'ontorelcat_api_ontorelquery'.
-- Utilisé pour éviter les erreurs de duplication lors de la création d'un nouveau type.
drop type if exists ontorelcat_api_ontorelquery.table_id_result cascade;

-- Création du type 'table_id_result' dans le schéma 'ontorelcat_api_ontorelquery'.
...
create type "ontorelcat_api_ontorelquery".table_id_result as
    (
...
create
or replace function "ontorelcat_api_ontorelquery".get_onto_classes(
...
create
or replace function "ontorelcat_api_ontorelquery".get_onto_object_property(
    p_ontorel_uuid varchar,
    p_language_code varchar default 'en'
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_onto_data_property(
    p_ontorel_uuid varchar,
    p_language_code varchar default 'en'
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_onto_datatypes(
    p_ontorel_uuid uuid,
    p_language_code varchar default 'en'
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_class_axioms(
    p_ontorel_uuid uuid,
    p_language_code varchar default 'en'
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_data_axioms(
    p_ontorel_uuid uuid,
    p_language_code varchar default 'en'
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_class_inheritance(
    p_ontorel_uuid uuid
)
returns table (
...
create
or replace function "ontorelcat_api_ontorelquery".get_class_by_iri(
    p_iri varchar,
    p_ontorel_uuid uuid,
    p_language_code varchar default 'en'
)
...
create
or replace function "ontorelcat_api_ontorelquery".get_label(table_id_ character varying, ontorel_uuid_ character varying,
...
create
or replace function "ontorelcat_api_ontorelquery".get_definition(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                code  character varying,
                value character varying
...
create
or replace function "ontorelcat_api_ontorelquery".get_data_type(iri_ varchar, ontorel_uuid_ varchar)
    returns varchar
    language sql
    security definer
begin
...
create
or replace function "ontorelcat_api_ontorelquery".get_object_property_domain(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                class_iri    varchar,
                property_iri varchar
...
create
or replace function "ontorelcat_api_ontorelquery".get_object_property_range(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                class_iri    varchar,
                property_iri varchar
...
create
or replace function "ontorelcat_api_ontorelquery".get_object_property(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                table_id varchar
            )
...
create
or replace function "ontorelcat_api_ontorelquery".get_data_property_domain(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                class_iri    varchar,
                property_iri varchar
...
create
or replace function "ontorelcat_api_ontorelquery".get_data_property_range(iri_ varchar, id_ontorel_ varchar)
    returns table
            (
                class_iri    varchar,
                property_iri varchar
...
create
or replace function "ontorelcat_api_ontorelquery".get_table_id_class(iri_ varchar, ontorel_uuid_ varchar)
    returns table
            (
                table_id varchar
            )
...
-- Sortie :
--     SETOF "ontorelcat_api_ontorelquery".table_id_result : Renvoie une table avec les colonnes suivantes :
...
create
or replace function "ontorelcat_api_ontorelquery".get_table_id_class_op(
    class_iri_ varchar,
    property_iri_ varchar,
    ontorel_uuid_ varchar
)
    returns SETOF "ontorelcat_api_ontorelquery".table_id_result
    language sql
    security definer
begin
    atomic
...
--         - class_table_id : Identifiant de la table correspondant à la classe.
create
or replace function "ontorelcat_api_ontorelquery".get_table_id_class_op_class(
    class_iri_ varchar,
    property_iri_ varchar,
    range_iri_ varchar,
    ontorel_uuid_ varchar
...
create
or replace function "ontorelcat_api_ontorelquery".get_table_id_class_dp(class_iri_ varchar, property_iri_ varchar, ontorel_uuid_ varchar)
    returns SETOF "ontorelcat_api_ontorelquery".table_id_result
    language sql
    security definer
begin
...
create
or replace function "ontorelcat_api_ontorelquery".get_class_hierarchy_detailed(
    p_class_iri varchar,
...
--         - table_id : Identifiant de la table correspondant à la classe.
create
or replace function "ontorelcat_api_ontorelquery".get_class_hierarchy_table_ids(
    p_class_iri varchar,
    p_inheritance_level int,
    p_ontorel_uuid varchar
)
